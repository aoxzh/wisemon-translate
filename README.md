...
<div align="center">
  ...
  <p><strong>Private bilingual translation for web pages, long text, PDFs, and video subtitles.</strong></p>
  <p>
...
- YouTube bilingual subtitle overlay with popup controls, track preference, nearby/full-video pre-translation, transcript panel, per-video cache, search, copy, multiple subtitle styles, and VTT export. Netflix and Bilibili timed subtitles are also supported.
...
- Local-only settings, API credentials, cache, and privacy-redacted logs.
...
- API keys are kept in local extension storage and are excluded from settings exports.
- Page context and persisted diagnostic logs use the same sensitive-data masking rules as translation text.
...
Packaging produces separate `dist/wisemon-translate-v<version>-chrome.zip` and `dist/wisemon-translate-v<version>-firefox.zip` archives.
...
- YouTube 字幕双语覆盖层并支持导出 VTT；同时支持 Netflix 与 Bilibili 定时字幕。
...
- API Key 保存在浏览器扩展存储中。
- API Key 仅保存在本地扩展存储中，导出设置时会自动移除全部凭证。
- 页面上下文和持久日志会使用与翻译正文一致的敏感数据脱敏规则。
- 翻译请求直接发送到你配置的接口。
...
打包会分别生成 `dist/wisemon-translate-v<version>-chrome.zip` 和 `dist/wisemon-translate-v<version>-firefox.zip`。
